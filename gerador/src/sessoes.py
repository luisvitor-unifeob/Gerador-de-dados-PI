...
def calcular_quantidade_sessoes(data_criacao, idade):
    """
    Calcula a quantidade de sessões do jogador.

    Contas antigas tendem a possuir mais sessões,
    mas jogadores diferentes possuem níveis
    ...
    # automaticamente centenas de sessões.
...
def gerar_datas_inicio(data_criacao, quantidade):
    """
    Gera os possíveis horários de início
    das sessões.

    Existe um pequeno peso para datas
    mais recentes, evitando uma distribuição
...
def escolher_preferencias_jogador():
    """
    Cada jogador possui alguns gêneros
    e plataformas preferidos.

    Isso evita que todos os jogadores
...
def escolher_genero(generos_preferidos):
    """
    Na maioria das sessões o jogador
    utiliza um dos seus gêneros preferidos.

    Ocasionalmente ele pode jogar
    outro gênero.
    """
...
def escolher_plataforma(genero_jogo, plataformas_preferidas):
    """
    Escolhe uma plataforma compatível
    com o gênero da sessão.
    """
...
def calcular_niveis(indice, quantidade_sessoes, nivel_atual):
    """
    Distribui a evolução de nível
    durante as sessões.

    A última sessão termina no
    nivel_jogador atual.
    """
...
def gerar_sessoes_jogador(jogador, proximo_sessao_id):
    ...
    # de saber quantas sessões realmente
    # foram criadas.
...
def gerar_arquivo_sessoes(caminho_jogadores):
    """
    Lê o CSV de jogadores e gera
    o CSV contendo as sessões.
    """
