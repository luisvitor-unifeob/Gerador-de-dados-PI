...
ARQUIVO_ESTADOS = (
    GC_PASTA_BASE / "codigos_divisoes_administrativas.txt"
)
...
def carregar_estados():
    GC_PASTA_BASE.mkdir(exist_ok=True)
...
def gerar_jogador(player_id):
    ...
    genero_jogo = random.choice(GENEROS_JOGO)
    plataforma = random.choice(PLATAFORMAS[genero_jogo])

    max_sessoes = max(
        1, int(dias_conta * random.uniform(0.05, 0.65))
    )
    ...
    tempo_min, tempo_max = (
        TEMPO_SESSAO_MEDIO[genero_jogo]
    )
    tempo_medio = random.uniform(
        tempo_min, tempo_max
    )
    horas_jogadas = round(
        numero_sessoes * tempo_medio, 1
    )

    nivel = min(
        100, max(1, int(horas_jogadas / 25) + 1)
    )
    ...
    if random.random() < 0.45:
        ...
        compras = random.randint(1, limite_compras)
